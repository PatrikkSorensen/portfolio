...
import sale_service_setup from './code-snippets/test-api/setup'
import sale_service_test_setup from './code-snippets/test-api/setup-test'

const projects = [
{
	id: 0, 
	cover : 'sales-api/cover.png',
	source: undefined,
	name: 'Creating a testable http service in Laravel',
	tags: ['Test Driven Development', 'Laravel', 'Mocking HTTP Requests', 'Integration testing'],
	role: ['Developer'], 
	created_with: ['php', 'laravel', 'Mockery', 'phpunit'], 
	date: 2017, 
	collaborators: ['Jacob Emcken'],
	content:  [
		{
			paragraph: [
				{
					text: "A work assignment at "
				},
				{
					link: {
						target: "http://www.cubs.dk/",
						label: "Casalogic Utilities Billing System (CUBS)"
					}
				}, 
				{
					text: ", was to create an API that would communicate with with a thin API wrapper a customer had made (a customer in cubs, is in this case an energy distributor)."
				},
				{
					text: " The four main endpoints the service should be used is: "
				},
			], 
		},
		{
			bulletlist: [
				...
				'UpdateSale: updates a sale, along with alle sales products',
				'DeleteSale: deletes sale',
				'RetrieveSalesStatus: retrieves status of sale',
			]
		},
		{
			italicText: [
				{
					text: 'This project was made at Casalogic and have been radically changed in order to not expose any unintended intellectual properties. As such, the code below has no relation with the cubs codebase, but reflects the approach and techniques that was used.'
				}, 
			]
		},
		{
		...
		},
		{
			paragraph: [
				{
					text: "The main things i like with Test driven development (TDD) is the reassurance that the code works exactly as intended with good and bad datasets, and the maintainability that follows. Each time you make a change, you can run the tests to see if everything still works."
				}
			], 
		},
		{
			paragraph: [
				...
				{
					link: {
						target: "http://docs.mockery.io/en/latest/",
						label: "Mockery, "
					}
				}, 
				{
					link: {
						target: "https://phpunit.de/",
						label: "phpunit "
					}
				}, 
				{
				...
				{
					link: {
						target: "http://docs.guzzlephp.org/en/stable/",
						label: "guzzle client "
					}
				},
				{
					text: "for executing the actual http requests. This is a handful of dependencies, but each of them lets you test the service fully. The context of the work assignment is that we have an imaginary customer that has an API where they would like to know about any creations, updates or deletions of sales that is created in our system."
				},
			], 
		},
		{
			paragraph: [
				{
					text: "Let's start by creating a boilerplate for the service and test itself"
				}
			], 
		},
		{
			code:	{
				language: 'php',
				contents: sale_service_setup
			}
		},
		{
			code:	{
				language: 'php',
				contents: sale_service_test_setup
			}
		},
		{
			paragraph: [
				{
					text: "The service is just empty functions where we expect the client being given in the constructor. The test cases match the function calls, but a mock function has been added that creates the service with a mocked guzzle client. We add our expectatations in the same function we create the mocked client."
				}
			], 
		},
	]
